...
A (predecessor) query is answered simply by performing binary search on every run. To control the cost, however, the algorithm makes sure that the current size $s$ is at least $i \Log i$ before processing the $i$-th ($i \ge 1$) query. If the condition is not met, an overhaul is invoked to increase $s$ to the nearest power of 2 at least $i \Log i$. This implies $s \le 2 \cdot i \log i$. Indeed, if $s$ was increased right before answering the $i$-th query, then obviously $s \le 2 \cdot i \log i$, Otherwise, suppose that $s$ was increased to the current value right before answering the $i_{last}$-th query; then $s \le 2 \cdot i_{last} \log i_{last} < 2 i \log i$.

\vgap

We can now assert that the $i$-th query entails a cost of $O(\fr{n}{s} \Log s) = O(\fr{n}{i \Log i} \cdot \log (i \Log i)) = O(n/i)$ time. It thus follows that the total cost of $r$ queries is $O(n \sum_{i=1}^r \fr{1}{i}) = O(n \Log r)$. As the final run size $s$ is at most $2r \Log r$, we can conclude that the algorithm processes $r$ queries in $O(n \Log r)$ time.
...
\section{A Universal Algorithm for $(\log n, \log n)$ Spectrum Indexable Problems} %\label{sec:reductions:2}
...
\begin{theorem} \label{thm:main}
    ...
    Every $(\log n, \log n)$ spectrum indexable problem admits a DDS algorithm that can process any $r \in [1, n]$ queries in $O(n \cdot \Log r)$ time.
\end{theorem}
%}


%\begin{proof}
    Next, we prove the theorem by explicitly describing a DDS algorithm achieving $\time(n ,r) = O(n \cdot \Log r)$. Assume, w.l.o.g., that $n$ is a power of 2. Define $B(n) = \log n$ and $Q(n) = \log n$.

    \vgap

    Our algorithm executes in {\em epochs}. At the beginning of the $i$-th ($i \ge 1$) epoch, we set
    \myeqn{
        s = 2^{2^i} \nn
    }

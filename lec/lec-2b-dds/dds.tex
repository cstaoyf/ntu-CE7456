...
{\em Deferred data structuring} (DDS) asks the question: if we are to answer $r$ queries in an {\em online} manner --- that is, the next query is given after the result of the previous query has been output --- what is the smallest total executing time (for answering $r$ queries)? This question arises in many big-data applications where large volumes of data are collected but only sparingly queried. In those environments, the problem size $n$ is huge such that even sorting is considered expensive and should be avoided as much as possible. Furthermore, it is impossible to predict how many queries --- namely, the integer $r$ mentioned earlier --- will need to be supported. Instead of constructing a complete index right away, a better algorithm is to carry out only a calculated portion of the construction during each query. If $r$ eventually reaches a certain threshold, the index will be created in full, but it is more likely that $r$ will stop at some value significantly lower than that threshold. The challenge is to ensure ``smoothness'': as $r$ grows, the total cost of all the $r$ queries {\em so far} ought to increase as slowly as possible.

\vgap

For a large class of DDS problems (such as predecessor search), it is known \cite{kmr88} that the total time of answering $r$ queries from a non-preprocessed dataset must be $\Omega(n \log (1+r))$ \cite{kmr88} for \uline{\em every} value $r \in [1, n]$. In today's lecture, we will discuss techniques for matching this lower bound.
...
We now formalize the DDS problem. Initially, an algorithm $\A$ is given the dataset $S$ in an array, where the elements are arbitrarily permuted. An adversary first chooses a query $q_1 \in \bQ$, and solicits the answer $\ans_{q_1}(S)$ from $\A$. Iteratively, for each $i \ge 2$, after the answer of the ($i-1$)-th query has been obtained, the adversary chooses the predicate $q_i \in \bQ$ for the next query and solicits $\ans_{q_i}(S)$ from $\A$. The adversary is permitted to observe the execution of $\A$ and, thus, capable of selecting a ``bad'' $q_i$ for $\A$.

\vgap

The algorithm $\A$ is said to guarantee {\em running time} $\time(n, r)$ if, for {\em every} $r \ge 1$, the first $r$ queries are processed with a total cost at most $\time(n, r)$.


\section{Merging and Cracking} \label{sec:prelim}

This section will introduce two popular techniques --- named {\em merging} and {\em cracking} --- for tackling DDS problems, using predecessor search as a representative example. In this problem, $S$ is a set of $n$ integers. Each query $q \in \mathbb{Q}$ is an integer; and $\ans_q(S)$ is the largest integer in $S$ at most $q$, or \ttt{null} if all the integers in $S$ are greater than $q$. This problem can be solved in $\time(n,r) = O(n \Log r)$ time. Next, we will prove this using both techniques. Our discussion will consider only $r \le n$ --- once we can achieve $\time(n, r) = O(n \Log r)$ for all $r \le n$, it is easy to show that there is a way to achieve $\time(n, r) = O(n \log r)$ for all $r \ge 1$ (think: why?).
...
We define a data structure problem parameterized by $(\bD, \bQ)$ to be $(B(n),Q(n))$ {\em spectrum indexable} if the following property holds on every dataset $S \subseteq \mathbb{D}$:
...
%Recall that a data structure problem parameterized by $(\bD, \bQ)$ is {\em decomposable} if, for any disjoint sets $S_1, S_2 \subseteq \mathbb{D}$ and any query $q \in \mathbb{Q}$, it is possible to derive $\ans_q(S_1 \cup S_2)$ from $\ans_q(S_1)$ and $ \ans_q(S_2)$ in constant time.
%
%
% For this purpose, we introduce two problem classes relevant to our discussion:
% \myitems{
%     \item A problem instance is {\em decomposable} if, for any disjoint sets $S_1, S_2 \subseteq \mathbb{D}$ and any predicate $q \in \mathbb{Q}$, it is possible to derive $\ans_q(S_1 \cup S_2)$ from $\ans_q(S_1)$ and $ \ans_q(S_2)$ in constant time.
%
%     \item We define a problem instance to be $(B(n),Q(n))$ {\em spectrum indexable} if it possesses the following property for
%     every dataset $S \subseteq \mathbb{D}$: for every integer $s \in [|S|]$, it is possible to construct a data structure on $S$ in $O(|S| \cdot B(s))$ time that can answer any query in $O(\fr{|S|}{s} \cdot Q(s))$ time. The term ``spectrum indexable'' is chosen to reflect the ability to build a ``good'' index structure --- as far as functions $B(n)$ and $Q(n)$ are concerned --- for the whole spectrum of the parameter $s$.
% }
...
\section{A Universal Algorithm} %\label{sec:reductions:2}
...
\begin{theorem} \label{thm:main}
    ...
    Every $(O(\log n), O(\log n))$ spectrum indexable problem admits a DDS algorithm that can process any $r \ge 1$ queries in $O(n \cdot \Log r)$ time.
\end{theorem}
%}


\begin{proof}
    Assume, w.l.o.g., that $n$ is a power of 2. Our algorithm executes in {\em epochs}. At the beginning of the $i$-th ($i \ge 1$) epoch, we set
    \myeqn{
        s = 2^{2^i} \nn
    }
    and create a structure $\T$ --- the structure promised by $(B(n), Q(n))$ spectrum indexability --- on $S$ in $O(n \cdot B(s))$ time. The structure allows us to answer any query  in $O(\fr{n}{s} \cdot Q(s))$ time. The $i$-th epoch finishes after $\ceil{s / Q(s)}$ queries\footnote{In practice, our algorithm may be slightly improved by making this number $\ceil{s \cdot B(s) / Q(s)}$, but this will not be necessary for the purpose of proving Theorem~\ref{thm:main}} are answered {\em during} the epoch. These queries demand a total cost of
    \myeqn{
        \lc \fr{s}{Q(s)} \rc \cdot O\Big(\fr{n}{s} \cdot Q(s) \Big) &=& O(n). \nn
    }

    It is clear from the above that the total computation time of the $i$-th epoch is $O(n \cdot B(s)) = O(n \cdot 2^i)$. As $n \cdot 2^i$ doubles when $i$ increases by 1, the overall cost of answering $r$ queries is $O(n \cdot 2^{h^*})$, where $h^*$ is the number of epochs needed. Precisely, the value of $h^*$ is the smallest integer $h \ge 1$ satisfying two conditions:
    \myitems{
        \item {\bf C1:} $2^{2^h} \le n$ (the value of $s$ must not exceed $n$);
        \item {\bf C2:} $
        \sum_{i=1}^h \lc \fr{2^{2^i}}{Q(2^{2^i})} \rc \ge r$ (the number of queries that can be answered by $h$ epochs must be at least $r$).
    }

    As our goal is to find an upper bound for $h$, we replace condition {\bf C2} with a more stringent condition: $2^{2^h} / Q(2^{2^h}) \ge r$. Because $Q(n) = O(n^{1-\eps})$, we know $Q(n) \le \alpha \cdot n^{1-\eps}$ for some constant $\alpha > 0$. With this, we modify {\bf C2} further into an even more stringent inequality:
    \myeqn{
        \fr{2^{2^h}}{\alpha \cdot (2^{2^h})^{1-\eps}} = \fr{(2^{2^h})^{\eps}}{\alpha} \ge r
        &\Leftrightarrow&
        2^{2^h} \ge (\alpha \cdot r)^{1/\eps}. \label{eqn:proof-lmm-reductions-2:help1}
    }
    Let $H$ represent the smallest integer $h \ge 1$ validating \eqref{eqn:proof-lmm-reductions-2:help1}. This means
    \myeqn{
        2^{2^{H-1}} < (\alpha \cdot r)^{1/\eps} &\Leftrightarrow& 2^{2^H} < (\alpha \cdot r)^{2/\eps}. \label{eqn:proof-lmm-reductions-2:help2}
    }
    When $2^{2^{H}} \le n$, the above argument assures us that the number of epochs $h^*$ is at most $H$ (the inequality $2^{2^{H}} \le n$ implies that $2^{2^{h^*}} \le n$, obeying Condition {\bf C1}). In this case, all epochs incur a total cost of $O(n \cdot 2^{h^*}) = O(n \cdot 2^{H})$, which is $O(n \Log r)$ by \eqref{eqn:proof-lmm-reductions-2:help2}.

    \vgap

    The above argument does not work if $2^{2^{H}} > n$. However, when this happens, we know from \eqref{eqn:proof-lmm-reductions-2:help2} that $(\alpha \cdot r)^{2/\eps} > 2^{2^H} > n$, leading to $r > n^{\eps/2} / \alpha$. As soon as $r$ reaches $\ceil{n^{\eps/2} / \alpha}$ --- the {\em snapping point}  --- we create a structure $\T$ on the whole $S$ in $O(n \log n) = O(n \log r)$ time, and use it to answer every subsequent query in $O(Q(n))$ time until $r = \min \{n, \fr{c \cdot n \log n}{Q(n)}\}$. The queries after the snapping point require a total cost of $O(n \log n) = O(n \log r)$. We thus have obtained an algorithm that guarantees $\time(n, r) = O(n \Log r)$ for all $r \le \min \{n, \fr{c \cdot n \log n}{Q(n)}\}$, completing the proof of Theorem~\ref{thm:main}.
\end{proof}

...
Let $\tm_S(Q)$ be the time of an algorithm $\A$ in answering a query with search value $Q$ on a dataset $S$. Note that both $S$ and $Q$ are random variables and, hence, so is $\tm_S(Q)$. We define the {\em average time complexity} of $\A$ over $\D$ as
\myeqn{
    \expt_{Q, S} [\tm_S(Q)]. \nn
}
...
Our data structure consists of (i) the values $a, b$ and $n$, (ii) the sorted array $A$, and (iii) the function $\hat{r}_S$. In particular, we store $\hat{r}_S$ in an array of size $n$ such that, for any $q \in I_i$ with $i \in [n]$, we can return $\hat{r}_S(q)$ in constant time. The total space consumption is $O(n)$ in the worst case.
...
\begin{theorem} \label{thm:main}
    The average query time of the algorithm in Section~\ref{sec:query} is
    \myeqn{
        O\Big(\log \Big(U \cdot \sum_{q \in [U]} (f(q))^2 \Big)\Big). \nn %\label{eqn:main}
    }
\end{theorem}

For example, when $\D$ is uniform over $[U]$, we have $f(x) = 1/U$ for all $x \in [U]$, in which case the average query time is $O(\log (U \cdot (U \cdot 1/U^2))) = O(1)$. More generally, the average query complexity is $O(\log t)$ if $f(x) \le t/U$ for all $x \in [U]$ --- for $t$ is sub-polynomial in $n$, the structure strictly improves the BST (in terms of average running time).
...
Each interval $J_j$ ($j \in [n]$) has a length $(U-1)/n$, which is at least that of an interval $I_i$ ($i \in [n]$) defined in \eqref{eqn:Ii}. The proposition below follows immediately:
...
\myeqn{
    p_j = \Pr_{X \sim \D} [X \in J_j] = \sum_{q \in J_j} f(q).
...
\subsection{Proof of Theorem~\ref{thm:main}} \label{sec:analysis:thm-main}

%To establish Theorem~\ref{thm:main} using Lemma~\ref{lmm:anayslis-core}, we need

% For each $j \in [n]$, we use $|J_j|$ to represent the number of integers covered by $J_j$. It is clear that $|J_j| \le U/n$.

%\vgap

By the Cauchy-Schwarz inequality\footnote{$(\sum_{i=1}^n x_i y_i)^2 \le \sum_{i=1}^n x_i^2 \cdot \sum_{i=1}^n y_i^2 $.}, we have for each $j \in [n]$:
\myeqn{
    p_j^2 = \Big(\sum_{q \in J_j} f(q) \Big)^2
    \le
    \Big(\sum_{q \in J_j} 1 \Big)
    \cdot
    \Big(\sum_{q \in J_j} (f(q))^2 \Big)
    \le
    O\Big( \fr{U}{n} \Big) \cdot \sum_{q \in J_j} (f(q))^2 \nn
}
where the last step used the fact that $J_j$ contains $O(U/n)$ integers. Plugging the above into Lemma~\ref{lmm:anayslis-core} yields:
\myeqn{
    \expt_{Q,S} [\Delta_S(Q)] &=& O(n) \cdot \sum_{j=1}^n O\Big( \fr{U}{n} \Big) \cdot \sum_{q \in J_j} (f(q))^2 \nn \\
    &=& O(U) \cdot  \sum_{j=1}^n \sum_{q \in J_j} (f(q))^2 \nn \\
    &=& O(U) \cdot  \sum_{q \in [U]} (f(q))^2. \label{eqn:thm-main:h1}
}

The average running time of the query algorithm in Section~\ref{sec:query} is bounded by
\myeqn{
    O \Big(
        \expt_{Q,S} \log \Delta_S(q)
    \Big)
    \le
    O \Big(
        \log \expt_{Q,S} \Delta_S(q)
    \Big) \nn
}
where the inequality is due to Jensen's inequality. The claim in Theorem~\ref{thm:main} now follows from the above and \eqref{eqn:thm-main:h1}.

\section{Remarks}

The structure and analysis presented were developed in \cite{cyl+25}.

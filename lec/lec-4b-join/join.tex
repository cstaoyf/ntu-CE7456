...
Let $\Q$ be a join with schema graph $\H = (\V, \E)$. Remarkably, the proof of the AGM bound inspires an algorithm for computing $\join(\Q)$. Set $d = |\V|$ and fix an arbitrary order of the attributes in $\V$: $X_1, X_2, ..., X_d$. We aim to solve the following {\em residual join computation problem}:
...
\extraspacing {\bf Analysis.} We make the following assumptions in our analysis:
\myitems{
    \item {{\bf A1:}} Given a tuple $\bm{u}$ over a set $U = \set{X_1, ..., X_t}$ where $t \in [0, d]$ and a relation $R \in \Q$, we can compute $|R(\bm{u})|$ in $O(1)$ time.

    %$R(\bm{u})$ --- defined in \eqref{eqn:alg-proof:residual-rel} --- in $O(1 + |R(\bm{u})|)$ time and

    \item {{\bf A2:}}  Given a tuple $\bm{u}$ over a set $U = \set{X_1, ..., X_t}$ where $t \in [0, d-1]$, a relation $R \in \Q$, and a value $x$, we can test whether $R(\bm{u})$ has a tuple whose value on $X_{t+1}$ is $x$.
}
These assumptions can be removed by slowing down the running time in the next lemma by at most a logarithmic factor (details left to you).
...
\begin{proof}
   ...
   time as follows. First, spend $O(1)$ time to find the edge $e^* = \argmin_{e \in \E: X_i \in e} |R_e(\bm{u})|$ (due to assumption {\bf A1}). Then, for each tuple in $R_{e^*}(\bm{u})$, we can test in $O(1)$ time whether the tuple's value on $X_i$ belongs to $\I$ (due to assumption {\bf A2}). We show that \eqref{eqn:alg:residual:1} is bounded by \eqref{eqn:alg:residual-time}. Because $\W$ is a fractional edge cover of $\H = (\V, \E)$, it holds that
    \myeqn{

...
\begin{lemma} \label{lmm:agm-proof:residual}
    Fix an arbitrary order of the attributes in $\V$: $X_1, X_2, ..., X_d$ where $d = |\V|$. Let $t \in [0, d]$ be an integer, and $\bm{u}$ a tuple over $U = \set{X_1, ..., X_t}$ such that $R(\bm{u}) \ne \emptyset$ for every $R \in \Q$. We have
    \myeqn{
        |\join(\Q(\bm{u}))|
        &\le&
...
Specially, if $t = 0$, then $U = \emptyset$ and $\Q = \Q(\bm{u})$ (because $R(\bm{u}) = R$ for all $R \in \Q$). Hence, Theorem~\ref{thm:agm} is a corollary of the above lemma with $U = \emptyset$.
...
% Because $\W$ is a fractional edge cover of $\H = (\V, \E)$, it holds that
% \myeqn{
...
\extraspacing {\bf Algorithm.} The base case $t = d$ can be trivially settled in $O(1)$ time. Inductively, suppose that we can solve the problem for $t = i$ for some $i \in [d]$. We solve the problem for $t = i-1$ as follows:
\myitems{

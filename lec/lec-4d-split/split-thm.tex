...
\boxminipg{\linewidth}{
    \begin{center}
        \vspace{3mm}
    {\Large
    (CE7456 Lecture Notes 11) A Geometric View of Natural Joins } \\[2mm]
    {\large Yufei Tao}
    \vspace{3mm}
    \end{center}
}

Today, we discuss a theorem that unveils an intrinsic mathematical structure of natural joins. This structure establishes a geometric, hierarchical view over the tuples in the join result, which enables simple algorithms for both computing the join and sampling from its result.
...
which is the total number of tuples in the relations of $\Q$.
...
We refer to the box $[\In]^d$ as the {\em join space}. Every tuple of $\join(\Q)$ can be seen as a point in this space. The converse, however, does not hold: not every point in $[\In]^d$ necessarily corresponds to a tuple of $\join(\Q)$.
...
where $\Q(B)$ is defined in \eqref{eqn:Q_B}. This indicates a simple way to compute $\join(\Q)$. Generate the entire $\T$, which (by property {\bf P4}) requires $O(\agm_{\W^*} \log \In)$ CPU time. By property {\bf P1}, each leaf $B$ of $\T$ has an AGM value at most 1; hence, we can compute $\join(\Q(B))$ in $O(1)$ time (assumption {\bf A4}). This yields a join reporting algorithm with running time $O(\agm_{\W^*} \cdot \log^2 \In)$.
...
\myeqn{
    \Pr[B^+ = B_1] &=& \fr{\agm_{\W^*}(B_1)}{\agm_{\W^*}(B)} \label{eqn:apps:B+_1} \\
    \Pr[B^+ = B_2] &=& \fr{\agm_{\W^*}(B_2)}{\agm_{\W^*}(B)} \label{eqn:apps:B+_2} \\
% }
% \vspace{-4mm}
%\myeqn{
...
\myeqn{
    \agm_{\W^*}(B) = 1. \label{eqn:apps:sample-help1}
}
To see why, note that $\agm_{\W^*}(B) \le 1$ by property {\bf P1}, while, on the other hand, $\agm_{\W^*}(B) \ge |\join(\Q(B))| \ge 1$. Let $B_0, B_1, ..., B_h$ be the nodes on the path of $\T$ from the root to $B$ (i.e., $B_0$ is the root and $B_h = B$). The probability that \ttt{sample} outputs $\bm{u}$ is
\myeqn{
    \prod_{i=1}^h \fr{\agm_{\W^*}(B_i)}{\agm_{\W^*}(B_{i-1})}
    &=&
    \fr{\agm_{\W^*}(B)}{\agm_{\W^*}([\In]^d)} \nn \\
...
In expectation, $\fr{\agm_{\W^*}}{\Out}$ repeats of the algorithm are needed to acquire a sample.

\vgap

A special case occurs when $\Out = 0$ --- this will force us to repeat the algorithm forever. To remedy the issue, we stop using \ttt{sample} after $\agm_{\W^*}$ repeats and revert to a worst-case optimal join algorithm (e.g., \cite{n18}) to compute $\join(\Q)$ in $O(\agm_{\W^*})$ time, which will either confirm $\Out = 0$ or allow us to obtain a random tuple of $\join(\Q)$ with no extra cost.

\vgap

We have not mentioned any data structure. Indeed, the \ttt{sample} algorithm itself requires no data structures. Nevertheless, to implement a call to the polymatroid oracle in $O(\log \In)$ time, one still needs a data structure of $O(\In)$ space, as will be discussed in Section~\ref{sec:boxtree:discussion}. This makes the expected join sampling time $O(\fr{\agm_{\W^*}}{\max\{1, \Out\}} \log^2 \In)$, which is worse than the state of the art \cite{wt26} by only a factor of $O(\log^2 \In)$.

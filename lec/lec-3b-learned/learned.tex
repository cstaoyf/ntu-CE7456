...
\myeqn{
    p_j^2 = \Big(\sum_{q \in J_j} f(j) \Big)^2
    \le
    \Big(\sum_{q \in J_j} 1 \Big)
    \cdot
    \Big(\sum_{q \in J_j} (f(j))^2 \Big)
    \le
    O\Big( \fr{U}{n} \Big) \cdot \sum_{q \in J_j} (f(j))^2. \nn
}
Plugging the above into Lemma~\ref{lmm:anayslis-core} yields:
\myeqn{
    \expt_{Q,S} [\Delta_S(Q)] &=& O(n) \cdot \sum_{j=1}^n O\Big( \fr{U}{n} \Big) \cdot \sum_{q \in J_j} (f(j))^2 \nn \\
    &=& O(U) \cdot  \sum_{j=1}^n \sum_{q \in J_j} (f(j))^2 \nn \\
    &=& O(U) \cdot  \sum_{q \in [U]} (f(j))^2. \label{eqn:thm-main:h1}
}
...
The structure and analysis were presented in \cite{cyl+25}.

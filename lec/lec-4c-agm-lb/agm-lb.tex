...
Let us list the vertices in $\V$ as $v_1, v_2, ..., v_n$ and the edges in $\E$ as $e_1, e_2, ..., e_m$ (the vertex and edge orders are not important).
...
\minipg{0.8\linewidth}{
    ...
    \myeqn{
        ...
        \sum_{j \in [n]: v_j \in e_i} \bm{x}[j] \le \log N_{e_i} &&  \forall i \in [m]. \nn
    }
}
...
\minipg{0.8\linewidth}{
    ...
    \myeqn{
        ...
        \sum_{i \in [m]: v_j \in e_i} \bm{x}[j] \ge 1 &&  \forall j \in [n]. \nn
    }
}
...
For any join $\Q$ such that (i) the schema graph of $\Q$ is $\H$ and (ii) for each $e \in \E$, $|R_e| = N_e$, the AGM bound of $\Q$ is equivalent to
\myeqn{
    2^
    {\sum_{i=1}^m \bm{y^*}[i] \cdot \log N_{e_i}.}
    \label{eqn:proof:agm}
...
up to a constant factor. By \eqref{eqn:lp-opt}, the values in \eqref{eqn:proof:agm} and \eqref{eqn:proof:join-size} are the same.

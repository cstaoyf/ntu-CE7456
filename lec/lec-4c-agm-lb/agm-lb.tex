...
Let us list the vertices in $\V$ as $X_1, X_2, ..., X_n$ and the edges in $\E$ as $e_1, e_2, ..., e_m$ (the vertex and edge orders are not important).
...
We now obtain a primal LP of the form \eqref{eqn:lp:primal} and its dual LP of the form \eqref{eqn:lp:dual}. The primal LP can also be written as:
\minipg{0.8\linewidth}{
...
\minipg{0.8\linewidth}{
    ...
    \myeqn{
        ...
        \sum_{i \in [m]: X_j \in e_i} \bm{x}[j] \ge 1 &&  \forall j \in [n]. \nn
    }
}
...
For any join $\Q$ such that (i) the schema graph of $\Q$ is $\H$ and (ii) for each $e \in \E$, $|R_e| \le N_e$, the AGM bound of $\Q$ is equivalent to
\myeqn{
    2^
    {\sum_{i=1}^m \bm{y}^*[i] \cdot \log N_{e_i}.}
    \label{eqn:proof:agm}
}
is the optimal value of the dual LP, which is the value given in \eqref{eqn:lp-opt}. Next, we construct such a join $\Q$ whose result contains
\myeqn{
    2^
    {\sum_{j=1}^n \bm{x}^*[j]}
...
tuples such that, for any $x_{j_k} \in \dom(X_{j_k})$ where $k$ ranges in $[t]$, the relation $R_{e_i}$ contains a tuple $\bm{u}$ with $\bm{u}(X_{j_k}) = x_{j_k}$.

\vgap

By our construction, for any $x_j \in \dom(X_j)$ where $j \in [n]$, the result of the join $\Q$ must contain a tuple $\bm{u}$ satisfying $\bm{u}(X_j) = x_j$ for all $j \in [n]$.

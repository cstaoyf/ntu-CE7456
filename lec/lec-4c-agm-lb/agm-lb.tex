...
The results of both LPs must be bounded. Denote by $\bm{x}^*$ (resp., $\bm{y}^*$) an optimal solution to the primal (resp., dual) LP.

\vgap

For any join $\Q$ such that
\myitems{
    \item the schema graph of $\Q$ is $\H$, and
    \item for each $e \in \E$, $|R_e| = N_e$,
}
its AGM bound is equivalent

...
\section{Functions ``Growing At Least Linearly''}
...
\begin{theorem} \label{thm:log:main}
    ...
    Set $h = \lc \log_2 n \rc$. There is a semi-dynamic structure  that
    \myitems{
...
Let $S$ be the input set of elements. Set $n = |S|$ and $h = \lc \log_2 n \rc$. At all times, we partition $S$ into disjoint subsets $S_0, S_1, ..., S_h$ (some of which may be empty) satisfying:
\myeqn{
    |S_i| &=& \textrm{either 0 or $2^i$}. \label{eqn:rebuild:log:size}
}
for each $i \in [0, h]$ (think: why is such partitioning always possible?) Create a structure of $\Upsilon$ (as is supplied to Theorem~\ref{thm:log:main}) on each subset; denote by $\Upsilon_i$ the structure on $S_i$. Then, $\Upsilon_1, \Upsilon_2, ..., \Upsilon_h$ together constitute our semi-dynamic structure.
...
To answer a query $q$, we simply search all of $\Upsilon_1, ..., \Upsilon_h$. As the query is decomposable, we can obtain the answer on $S$ from the answers on $S_1, ..., S_h$ in $O(h)$ time. The overall query time, excluding the cost of outputting the result, is
\myeqn{
...
\myitems{
    \item If $i$ exists, destroy $\Upsilon_0, \Upsilon_1, ..., \Upsilon_{i-1}$ and move all the elements in $S_0, S_1, ..., S_{i-1}$, together with $e_\mit{new}$, into $S_i$. (Think: why doesn't this violate \eqref{eqn:rebuild:log:size}?) After this, $S_0, S_1, ..., S_{i-1}$ become empty. Build $\Upsilon_i$ on $S_i$.

    \item Otherwise, destroy $\Upsilon_0, \Upsilon_1, ..., \Upsilon_h$ and move all the elements in $S_0, S_1, ..., S_h$, together with $e_\mit{new}$, into $S_{h+1}$. After this, $S_0, S_1, ..., S_h$ become empty. Build $\Upsilon_{h+1}$ on $S_{h+1}$ and increase the value of $h$ by 1.
}

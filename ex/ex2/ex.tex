...
\myitems{
    \item (a) Divide $\real^2$ into $\sqrt{n}$ vertical slabs, each containing $\sqrt{n}$ points of $P$. An axis-parallel rectangle $q$ can be partitioned into
\myitems{
...
Hint: Talk to a LLM about ``the range tree'' and integrate it with the 1D solution given in the class.

\extraspacing {\bf Problem 3.} Let $P$ be a set of $n$ points in $\real^2$. Given an axis-parallel rectangle $q$, a {\em range query} returns $P \cap q$. In $O(n \log n)$ time, we can construct a so-called {\em kd-tree} that uses $O(n)$ space and answers a range query in $O(\sqrt{n} + k)$ time, where $k = |P \cap q|$.
...
\extraspacing {\bf Problem 4.} Describe how to turn the alias structure into a semi-dynamic structure that uses $O(n)$ space, supports an insertion in $O(\log n)$ amortized time, and can be used to draw a weighted sample in $O(1)$ time.

\vgap

Hint: Note that the update time is $O(\log n)$, rather than $O(\log^2 n)$! Furthermore, the sampling time is constant, rather than $O(\log n)$.

\extraspacing {\bf Problem 5* (Dynamic Set Sampling).} The set sampling problem is a special case of weighted sampling where each element carries the same weight (which can be assumed to be 1, w.l.o.g.). Describe a {\em fully} dynamic structure that uses $O(n)$ space, supports an insertion in $O(\log n)$ amortized time, and can be used to draw a sample in $O(1)$ expected time. You may assume a ``perfect'' hash structure that (i) allows you to decide whether an element belongs to a set in constant time and (ii) supports an insertion in constant time.

\vgap

Hint: The problem is hard! You need to support arbitrary insertions and deletions (rather than just FIFO updates)! For this purpose, you first need to figure out how to support deletions. Start with this problem: design a data structure of $O(n)$ space that supports a deletion in $O(1)$ time.
\end{document}

...
\myitems{
    \item Describe how to construct a data structure of $O(n)$ space that, given any axis-parallel rectangle $q$, can report $|P \cap q|$ in $O((k + 1) \log^2 n)$ time, where $k = |P \cap q|$. Note: the query time does not depend on $U$.

    \item Describe how to construct a data structure of $O(n)$ space that, given any axis-parallel rectangle $q$, can extract a point from $|P \cap q|$ uniformly at random in $O(\log^2 n)$ time. Again, note that the query time does not depend on $U$.
}
...
\extraspacing {\bf Problem 4.} (Sampling from Keyword Search) \\
Let $P$ be a set of points in $[U]^2$, where $U$ is an integer that fits in a word. Each point $p \in P$ carries a set $C(p)$ of {\em colors} (a.k.a.\ keywords), where each color is an integer. Given $m$ colors $c_1, c_2, ..., c_m$ and an axis-parallel rectangle $q$, a query returns a point drawn uniformly at random from those points $p \in P \cap q$ that carry at least one chosen color, namely, $C(p) \cap \set{c_1, ..., c_m} \ne \emptyset$. Different queries may differ in their $m$ values (and, of course, also in the color choices).

\vgap

Let $N = \sum_{p \in P} |C(p)|$. Describe a structure of $O(N)$ space that answers each query in $O(m \polylog N)$ expected time. You may assume the ``counting structure'' in Problem 1.

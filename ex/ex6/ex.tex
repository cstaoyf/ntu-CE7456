...
\extraspacing {\bf Problem 6.} In the MPC model, if a machine wants to broadcast a word to every other machine, it takes a load of $p$ naively. Describe an algorithm that performs the broadcasting in constant rounds with load $O(p^\eps)$ where $\eps$ can be any small positive constant.
\end{document}

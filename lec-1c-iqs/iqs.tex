...
The phenomenon has motivated {\em independent query sampling} (IQS) \cite{hqt14}, which aims to return only a sample set of the query result, with the requirement that the sample sets of all queries must be mutually independent. Next, we formalize the notion over range reporting.
...
Let $\T$ be a binary search tree (BST) on $S$ with the following properties:
\myitems{
    \item $\T$ has height $O(\log n)$.
    \item $\T$ has $n$ leaves each storing a distinct value in $S$ as the {\em key}.
    \item Every internal node $u$ in $\T$ has two children. The {\em key} of $u$ equals the smallest leaf key in the right subtree. The leaf keys in the left subtree of $u$ are less than the key of $u$.
}
...
\begin{figure}
    ...
    \figcapup
    \caption{The black nodes are the canonical nodes of the query interval $q = [x, y]$. The shaded triangles indicate the subtrees of the canonical nodes. }
    \label{fig:bst}
\end{figure}
...
\section{A Structure of $O(n \log n)$ Space} \label{sec:wrs:nlgn-space}

This section will reduce the time of an IQS query to $O(\log n + s)$ using a data structure of space $O(n \log n)$.

\vgap

Build a BST $\T$ on $S$ with the properties listed in Section~\ref{sec:basic:tree}. For each node $u$ in $\T$, we use $S(u) \subseteq S$ to denote the set of elements from $S$ that are stored at the leaves in the subtree of $u$. At each $u$, we store $w(u) = \sum_{e \in S(u)} w(e)$ and create an alias structure $A_u$ on $S(u)$. Because $A_u$ occupies $O(|S(u)|)$ space, the alias structures of all the nodes at the same level of $\T$ together use $O(n)$ space. The total space of the structure is therefore $O(n \log n)$.

\vgap

Next, we explain how to answer an IQS query parameterized by interval $q$ and sample size $s$. As in Section~\ref{sec:basic:tree}, we start by identifying a set $\C$ of canonical nodes for $q$ (see Figure~\ref{fig:bst}) in $O(\log n)$ time. Let the nodes in $\C$ be $u_1, u_2, ..., u_t$ for some $t = O(\log n)$. Remember that the sets $S(u_1), S(u_2), ..., S(u_t)$ constitute a partition of $S_q = q \cap S$, namely, they are mutually disjoint and their union is exactly $S_q$.

\vgap

The next step is to determine how many samples to take from each $S(u_i)$, $i \in [1, t]$. This is an instance of weighted sampling. Specifically, a weighted sample of $S_q$ should come from $S(u_i)$ with probability $w(u_i) / \sum_{j=1}^t w(u_j)$ for each $i \in [1, t]$. Let $s_i$ be the number of weighted samples to originate from $S(u_i)$, i.e., $s_1, ..., s_t$ are random variables with sum $s$. We can generate these random variables in $O(t + s) = O(\log n + s)$ time as follows. First, construct an alias structure on $\C$ in $O(t)$ time, by treating $w(u)$ as the weight for each $u \in \C$. Then, we draw $s$ weighted samples from $\C$ in $O(s)$ time, after which $s_i$ ($1 \le i \le t$) can be set to the number of occurrences of $u_i$ in those samples.
...
\begin{lemma} \label{lmm:wrs:nlogn}
    There is a structure of $O(n \log n)$ space that answers an IQS query in $O(\log n + s)$ time.
\end{lemma}

\section{Reducing the Space to $O(n)$} \label{sec:wrs:n-space}

Next, we improve the space consumption to $O(n)$ using a chunking idea. Divide $\real$ into interior-disjoint intervals $I_1, I_2, ..., I_g$ for some $g = \Theta(n / \log n)$ satisfying the following conditions:
\myitems{
...
It is easy to obtain such intervals with sorting. We will refer to each $\I_i$ as a {\em chunk} and define $w(\I_i) = \sum_{e \in \I_i} w(e)$.
...
See Figure~\ref{fig:chunk} for an illustration. Define $S_1 = S \cap q_1$, $S_2 = S \cap q_2$, and $S_3 = S \cap q_3$. For each $j \in [1, 3]$, define $w(S_j) = \sum_{e \in S_j} w(e)$. We can obtain $S_1$ and $S_3$ in their entirety in $O(\log n)$ time by reading the chunks $\I_a$ and $\I_b$ directly. For $S_2$, we cannot afford to enumerate its elements but can obtain $w(S_2)$ in $O(\log n)$ time from the range sum structure. To extract $s$ weighted samples from $S_q$, we first determine the number $s_j$ of samples that will come from $S_j$ for each $j = 1, 2$, and 3 (this can be easily done in $O(s)$ time). By resorting to the alias method, the $s_1$ and $s_3$ samples from $S_1$ and $S_3$ can be fetched in $O(\log n + s_1 + s_3)$ time. Acquiring $s_2$ samples from $S_2$ requires only a chunk-aligned query, which we already know how to solve in $O(\log n + s_2)$ time. We thus conclude that the overall query time is $O(\log n + s_1 + s_2 + s_3) = O(\log n + s)$.


\begin{figure}
    \centering
    \includegraphics[height=15mm]{./artwork/chunk}
    \figcapup
    \caption{Query processing with chunks}
    \label{fig:chunk}
\end{figure}
...
\begin{theorem} \label{thm:wrs}
    There is a structure of $O(n)$ space that answers an IQS query in $O(\log n + s)$ time.
\end{theorem}
